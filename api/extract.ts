...
import { runExtraction } from "./_lib/run";
import type { ExtractRequest } from "./_lib/run";

// Best-effort in-memory rate limiter. Serverless instances are ephemeral, so
// this caps abuse per warm instance rather than globally — a lightweight guard
// against runaway AI cost / proxy abuse. (A durable limiter would use the DB.)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Require a valid Supabase session — never an open endpoint.
