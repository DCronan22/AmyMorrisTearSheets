...
import type { StyleDetectRequest } from "./_lib/style-run.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const body = (req.body ?? {}) as StyleDetectRequest;
  try {
    const { status, payload } = await runStyleDetect(body);
    return res.status(status).json(payload);
  } catch {
    return res.status(500).json({ error: "Style detection failed unexpectedly." });
  }
}
